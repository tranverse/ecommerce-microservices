...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiErrorResponse> handleMalformedRequest(Exception exception, HttpServletRequest request) {
        return response(HttpStatus.BAD_REQUEST, ErrorCode.MALFORMED_REQUEST,
                "Request contains a malformed or unsupported value", List.of(), request);
    }
    ...
    ResponseEntity<ApiErrorResponse> handleOptimisticLock(
        ...
        return response(HttpStatus.CONFLICT, ErrorCode.DATA_INTEGRITY_CONFLICT,
                "Inventory changed concurrently; retry with fresh state", List.of(), request);
    }
    ...
    ResponseEntity<ApiErrorResponse> handleDataIntegrity(
        ...
        log.warn("Database constraint rejected the inventory operation", exception);
        return response(HttpStatus.CONFLICT, ErrorCode.DATA_INTEGRITY_CONFLICT,
                "The request conflicts with existing inventory data", List.of(), request);
    }
    ...
    ResponseEntity<ApiErrorResponse> handleNoResourceFound(
        ...
        return response(HttpStatus.NOT_FOUND, ErrorCode.ENDPOINT_NOT_FOUND,
                "The requested endpoint was not found", List.of(), request);
    }

    @ExceptionHandler(Exception.class)
    ResponseEntity<ApiErrorResponse> handleUnexpected(Exception exception, HttpServletRequest request) {
        log.error("Unhandled inventory service error", exception);
    ...
    private ResponseEntity<ApiErrorResponse> response(
        ...
        return ResponseEntity.status(status).body(new ApiErrorResponse(
                Instant.now(), status.value(), errorCode.name(), message, details,
